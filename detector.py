...
def detect_onsets(odf_rate, odf, options):
    ...
    threshold = 0.2 * np.max(odf)
    onsets = []
    min_distance = 0.05

    for i in range(1, len(odf)-2): # iterate through every frame
        if odf[i] > threshold:
            if odf[i-1] < odf[i] and odf[i] > odf[i+1]: # find peak
                onsets.append(i)

    onsets = np.array(onsets)
    onsets = onsets / odf_rate # convert from frame index to time
